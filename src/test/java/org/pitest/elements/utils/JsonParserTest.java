...
public class JsonParserTest {
  ...
  private PackageSummaryMap createPackageSummaryMap(
    ...
    for (final String fileName : mutantsByFile.keySet()) {
      ...
      final MutationTestSummaryData data =
          new MutationTestSummaryData(
              fileName, mutantsByFile.get(fileName), Collections.singletonList(classLines));
  ...
  private PackageSummaryMap createPackageSummaryMap(List<String> files) {
    ...
    for (String fileName : files) {
      ...
      final MutationTestSummaryData data =
          new MutationTestSummaryData(
              fileName, Collections.emptyList(), Collections.singletonList(classLines));
      map.update("package", data);
    }
    return map;
  }
  ...
  private JsonParser createTestee(final MockSourceLocator sourceLocator) {
    final Collection<SourceLocator> sourceLocators = Collections.singletonList(sourceLocator);
